...
## 1. 验收结果
- 已将平台页面与客户端脚本从 `/apps/delivery-app/src` 拆分到 `/apps/platform/src`：
...
- 已保留可访问入口并完成职责分离：
  - 配送员端工作台入口改为跳转 `/apps/platform/src/*.html`
  - 平台页面返回入口改为 `/apps/delivery-app/src/workbench.html`
- 未改变业务接口契约与权限校验：
...
## 3. 风险与后续
- 本地预览脚本已调整为以 `/.trellis` 为静态根目录，历史直链（如 `/workbench.html`）需切换到 `/apps/delivery-app/src/workbench.html`。
- 平台侧当前复用 `apps/delivery-app/src/auth-client.js`，后续可抽到 shared 前端模块以进一步降低跨端耦合。
