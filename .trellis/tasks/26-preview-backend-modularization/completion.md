...
- 与 task-25 对接完成：平台页面已从 delivery-app 拆分到 `/.trellis/platform/src`，脚本按独立端口 `5175` 启动并校验平台入口。
- 后端平台路由模块化：新增 `backend/src/platform-routes.js`，将 `/platform/*` 路由处理从 `server.js` 抽离为独立模块。
