...
> 实现参考：`/apps/delivery-app/src/workbench.html`、`workbench-client.js`、后端工作台聚合接口（若有）  
...
| **下一配送卡片字段与入口层级** | 中 | task-39 补齐过 §3.2.2 全字段；task-53 / task-54 依据产品决策回收到首页最小闭环，仅保留客户、地址、规格数量、应收金额，并将待配送入口收敛为卡片下单一入口 |
...
| **离线乐观收入与冲突策略** | 中 | task-59 后首页不再弹同步模态，改为“我的-离线数据”低频入口手动提交；与需求第 7 节全文未逐项对齐 |
...
| §2.1 区域 B | 「查看全部」待配送列表入口 | **已满足** | task-54：入口仍位于区域 B，但从卡片头部移到卡片下方，作为首页唯一待配送入口，链接仍指向 `delivery-complete.html?from=workbench&view=pending` |
| §3.1 收入卡 | 已收/待收/毛利/趋势 | 部分 | 已收/待收/合计已展示；毛利与趋势为 **占位**（task-38），真实口径与图表待财务/数据接口 |
| §3.2 下一配送 | 排序规则 | 部分 | 规则见 overview；需与列表接口契约对齐 |
| §3.2 下一配送 | 卡片字段全集 | 部分 | 原需求字段全集已在 task-39 打通过；task-53 基于首页聚合定位做产品收口，当前首页只保留客户/地址/规格数量/应收，完整处理转移到待配送页 |
...
| 2026-04-14 | task-53 / task-54：首页待配送卡片回收到最小关键信息，待配送入口改为卡片下方单一入口，底部操作区收敛为“快速开单 + 语音开单” |
| 2026-04-14 | task-59：首页移除自动同步弹层与重试交互，同步入口下沉到“我的-离线数据”并采用手动提交 |
