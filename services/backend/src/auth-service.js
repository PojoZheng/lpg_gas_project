...
function createOrGetUser(phone) {
  ...
  const regionCode = suffix === "01" || suffix === "02" ? "CN-GD-SZ" : "CN-DEFAULT";
  const companyId = suffix === "01" || suffix === "02" ? "COMP-SZ-A" : "COMP-DEFAULT";
  const companyName = suffix === "01" || suffix === "02" ? "深城燃气服务公司" : "多立恒默认燃气公司";
  if (!users.has(phone)) {
    users.set(phone, {
      id: crypto.randomUUID(),
      phone,
      nickname: `配送员${phone.slice(0, 3)}****${phone.slice(-4)}`,
      dealerId: `DEALER-${phone.slice(-4)}`,
      regionCode,
      companyId,
      companyName,
      sessions: [],
